...
export function ItemCard({ item, onOpen, index = 0 }: { item: CatalogItem; onOpen: () => void; index?: number }) {
  ...
  return (
    <motion.button
      ...
      onClick={onOpen}
      className="flex w-full items-center gap-3 rounded-xl2 border border-border bg-surface p-3 text-left shadow-card transition-colors"
    >
      <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-surface2 text-textdim">
        {item.image_url ? <img src={item.image_url} alt="" className="h-full w-full object-cover" /> : <IconBag className="h-6 w-6" />}
      </div>
      <div className="min-w-0 flex-1">
        <div className="truncate text-sm font-semibold text-text">{item.title}</div>
        <div className="mt-0.5 flex items-center gap-1.5">
          <span className="tabular-nums font-mono text-sm font-medium text-accent">{fmtMoney(item.price_uzs)}</span>
